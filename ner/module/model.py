...
def register_model(name):
    ...
    def register_model_cls(cls):
        ...
        if not issubclass(cls, TransformerBase):
            raise ValueError('Model ({} : {}) must extend TransformerBase'.format(name, cls.__name__))
...
class TransformerGRUAttnCRF(TransformerBase):
    ...
    def forward(self, x, y):
        ...
        x = PositionalEncoding(d_model=self.arg.embed_dim, max_len=self.arg.num_vocabs).forward(x)
