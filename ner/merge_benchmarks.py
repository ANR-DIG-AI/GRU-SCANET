...
class MergeCSVFiles:
    def __init__(self, input_dir, output_dir):
        ...
        self.labels = {}
        self.targeted_files = ['devel.csv', 'test.csv', 'train_dev.csv', 'train.csv']
            ...
            for sub_directory in Path(self.input_dir).iterdir():
                if sub_directory.is_dir():
                    ...
                    for i in range(data.shape[0]):
                        data.iloc[i]['NER'] = self.replacement(label=str(sub_directory),index=index, sequence=data.iloc[i]['NER'])
                    target_merged_data = pd.concat([target_merged_data, data], ignore_index=True)
                index = index + 1        
            output_file_name = f"{self.output_dir}{target.replace('.csv', '')}_merged.csv"
            target_merged_data.to_csv(output_file_name, index=False)
            print(f"Merged {len(target_merged_data)} rows into {output_file_name}")
        self.save_json(response_json=self.labels, file_name=self.output_dir+'labels.json')

# Exemple d'utilisation :
input_directory = "../data/processed/"
output_directory = "../data/processed/"
merger = MergeCSVFiles(input_directory, output_directory)
merger.merge()
